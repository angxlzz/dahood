...
if not Solara then
    ...
    if Game_Name == "South Bronx" and not Solara then
        ...
        for Index, Value in Services.ReplicatedStorage.Workspace.Homeless:GetChildren() do
            Value.Parent = Services.Workspace.HomelessPeople
        end

        for Index, Value in Services.Workspace.HomelessPeople:GetChildren() do
        ...
        for Index, Value in Services.ReplicatedStorage.Workspace.NPCs:GetChildren() do
            Value.Parent = Services.Workspace.NPCs
        end

        for Index, Value in Services.Workspace.NPCs:GetChildren() do
            Value:GetPropertyChangedSignal('Parent'):Connect(LPH_NO_VIRTUALIZE(function()
                task.wait()
                Value.Parent = Services.Workspace.NPCs
            end))
    ...
    LPH_JIT_MAX(function()
        ...
        if Game_Name == "South Bronx" then
            ...
            local GetCuttingBoard, GetBowl, GetPot, CheckHobo, GetHouse = LPH_NO_VIRTUALIZE(function()
                ...
                if Workspace.HomelessPeople:FindFirstChild(Name) then
                    local Prompt = Workspace.HomelessPeople:FindFirstChild(Name):FindFirstChildWhichIsA("ProximityPrompt", true)

                    if Prompt and Prompt.Enabled and Workspace.HomelessPeople:FindFirstChild(Name).LeftLowerLeg.Rotation.X == 90 then
                        return true
                    end
                end
            ...
            local Get_Hobo = LPH_NO_VIRTUALIZE(function()
                for i,v in Workspace.HomelessPeople:GetChildren() do
...
local Fonts = {}; do
    ...
    local Tahoma = RegisterFont("Tahoma", 400, "Normal", {
        Id = "Tahoma.ttf",
        Font = game:HttpGet("https://github.com/angxlzz/bronx/raw/refs/heads/main/fs-tahoma-8px%20(3).ttf"),
    })

    local Pixel = RegisterFont("Pixel", 400, "Normal", {
        Id = "Pixel.ttf",
        Font = game:HttpGet("https://github.com/angxlzz/bronx/raw/refs/heads/main/Pixel.ttf"),
    })
    ...
    -- Library element functions
        function library:window(properties)
            ...
            items[ "close button" ].InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.TouchTap then
                    open = not open 
...
local window = library:window({name = "bronx", suffix = ".lol", gameInfo = "bronx.lol"})
                    ...
                    task.spawn(LPH_NO_VIRTUALIZE(function()
                        ...
                        local suc, error = pcall(function()
                            ...
                            elseif Config.South_Bronx.Selected_Location == "Dirty Hobo 💩" then
                                if Workspace.HomelessPeople:FindFirstChild("RightLowerLeg", true) then
                                    local _Hobo = Workspace.HomelessPeople:FindFirstChild("RightLowerLeg", true).CFrame
                                    _Position = _Hobo
                                    Teleport(_Hobo)
                                else
                                    library.notifications:create_notification({
